...
define(MAX_W,  '300');                  // 投稿サイズ幅（これ以上はwidthを縮小
define(MAX_H,  '300');                  // 投稿サイズ高さ
...
define(MAX_RES, '10');                  // 強制sageになるレス数
...
define(MAX_PASSED_HOUR, '36');          // 強制sageまでの時間   0で強制sageなし
...
function form(&$dat,$resno,$admin=""){
  ...
  $dat .= '<table cellpadding=1 cellspacing=1>'.
          ...
          '<tr><td bgcolor=#eeaa88><b>コメント</b></td><td>'.
          '<textarea name=com cols="48" rows="4" wrap=soft></textarea></td></tr>';
  // hage 変更 2004.8.1
  ...
  // hage 変更ここまで
  $dat .= '<tr><td bgcolor=#eeaa88><b>削除キー</b></td><td>'.
...
function updatelog($resno=0){
  ...
  for($i = $st; $i < $st+PAGE_DEF; $i++){
    ...
    if($ext && is_file($img)){
      ...
      if (IMG_REFER) {
        $dat.="画像タイトル：<a href=\"".IMG_REF_DIR.$time.".htm\" target=_blank>$time$ext</a>-($size B)<br>$imgsrc";
      }else{
    ...
    for($k = $s; $k < count($treeline); $k++){
      ...
      if(in_array($host,$hostdat)){
        $dat .= "<blockquote>[<font color=#ff0000>$host</font>]<br>$com</blockquote>";
      }else{
        $dat .= "<blockquote>$com</blockquote>";
      }
      // hage 追加ここまで
...
function regist($name,$email,$sub,$com,$url,$pwd,$upfile,$upfile_name,$resto){
  global $path,$badstring,$badfile,$badip,$pwdc,$textonly,$noanime;
  ...
  if(strlen($resto) > 10) error("レス番号が長すぎます",$dest);
  if(strlen($url) > 100) error("URLが長すぎます",$dest);
...
function admin_chgthumb($pass){
  global $path,$stillGIF,$rep_thumb,$default_thumb;
